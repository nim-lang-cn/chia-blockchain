...
class WalletPuzzleStore:
    ...
    async def create(cls, db_wrapper: DBWrapper, cache_size: uint32 = uint32(600000), db="chia"):
        ...
        await self.db_connection[db].execute("CREATE INDEX IF NOT EXISTS ph on derivation_paths(puzzle_hash)")

        await self.db_connection[db].execute("CREATE INDEX IF NOT EXISTS pubkey on derivation_paths(pubkey)")

        await self.db_connection[db].execute("CREATE INDEX IF NOT EXISTS wallet_type on derivation_paths(wallet_type)")

        await self.db_connection[db].execute("CREATE INDEX IF NOT EXISTS wallet_id on derivation_paths(wallet_id)")

        await self.db_connection[db].execute("CREATE INDEX IF NOT EXISTS used on derivation_paths(wallet_type)")
    ...
    async def close(self, db="chia"):
        await self.db_connection[db].close()

    async def _init_cache(self, db="chia"):
        self.all_puzzle_hashes = await self.get_all_puzzle_hashes()

    async def _clear_database(self, db="chia"):
        cursor = await self.db_connection[db].execute("DELETE FROM derivation_paths")
        await cursor.close()
        await self.db_connection[db].commit()

    async def add_derivation_paths(self, records: List[DerivationRecord], in_transaction=False, db="chia") -> None:
        """
        ...
        try:
            ...
            cursor = await self.db_connection[db].executemany(
    ...
    async def get_derivation_record(self, index: uint32, wallet_id: uint32, db="chia") -> Optional[DerivationRecord]:
        """
    ...
    async def get_derivation_record_for_puzzle_hash(self, puzzle_hash: str, db="chia") -> Optional[DerivationRecord]:
        """
        Returns the derivation record by index and wallet id.
        """
        cursor = await self.db_connection[db].execute(
    ...
    async def set_used_up_to(self, index: uint32, in_transaction=False, db="chia") -> None:
        """
    ...
    async def puzzle_hash_exists(self, puzzle_hash: bytes32, db="chia") -> bool:
        """
    ...
    async def index_for_pubkey(self, pubkey: G1Element, db="chia") -> Optional[uint32]:
        """
    ...
    async def index_for_puzzle_hash(self, puzzle_hash: bytes32, db="chia") -> Optional[uint32]:
        """
    ...
    async def index_for_puzzle_hash_and_wallet(self, puzzle_hash: bytes32, wallet_id: uint32, db="chia") -> Optional[uint32]:
        """
        Returns the derivation path for the puzzle_hash.
        Returns None if not present.
        """
    ...
    async def wallet_info_for_puzzle_hash(self, puzzle_hash: bytes32, db="chia") -> Optional[Tuple[uint32, WalletType]]:
        """
        Returns the derivation path for the puzzle_hash.
        Returns None if not present.
        """
    ...
    async def get_all_puzzle_hashes(self, db="chia") -> Set[bytes32]:
        """
        Return a set containing all puzzle_hashes we generated.
        """

        cursor = await self.db_connection[db].execute("SELECT * from derivation_paths")
    ...
    async def get_last_derivation_path(self, db="chia") -> Optional[uint32]:
        """
        Returns the last derivation path by derivation_index.
        """

        cursor = await self.db_connection[db].execute("SELECT MAX(derivation_index) FROM derivation_paths;")
        row = await cursor.fetchone()
        await cursor.close()
    ...
    async def get_current_derivation_record_for_wallet(self, wallet_id: uint32, db="chia") -> Optional[DerivationRecord]:
        """
    ...
    async def get_unused_derivation_path(self, db="chia") -> Optional[uint32]:
        """
        Returns the first unused derivation path by derivation_index.
        """
        cursor = await self.db_connection[db].execute("SELECT MIN(derivation_index) FROM derivation_paths WHERE used=0;")
        row = await cursor.fetchone()
        await cursor.close()
