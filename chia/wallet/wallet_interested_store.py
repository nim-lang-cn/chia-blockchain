...
class WalletInterestedStore:
    ...
    async def create(cls, wrapper: DBWrapper, db="chia"):
        ...
        await self.db_connection[db].execute("CREATE TABLE IF NOT EXISTS interested_coins(coin_name text PRIMARY KEY)")
    ...
    async def _clear_database(self, db="chia"):
        cursor = await self.db_connection[db].execute("DELETE FROM puzzle_hashes")
        await cursor.close()
        cursor = await self.db_connection[db].execute("DELETE FROM interested_coins")
        await cursor.close()
        await self.db_connection[db].commit()

    async def get_interested_coin_ids(self, db="chia") -> List[bytes32]:
        cursor = await self.db_connection[db].execute("SELECT coin_name FROM interested_coins")
        rows_hex = await cursor.fetchall()
        return [bytes32(bytes.fromhex(row[0])) for row in rows_hex]

    async def add_interested_coin_id(self, coin_id: bytes32, in_transaction: bool = False, db="chia") -> None:
    ...
    async def get_interested_puzzle_hashes(self, db="chia") -> List[Tuple[bytes32, int]]:
        cursor = await self.db_connection[db].execute("SELECT puzzle_hash, wallet_id FROM interested_puzzle_hashes")
        rows_hex = await cursor.fetchall()
        return [(bytes32(bytes.fromhex(row[0])), row[1]) for row in rows_hex]

    async def get_interested_puzzle_hash_wallet_id(self, puzzle_hash: bytes32, db="chia") -> Optional[int]:
        cursor = await self.db_connection[db].execute(
    ...
    async def add_interested_puzzle_hash(
        self, puzzle_hash: bytes32, wallet_id: int, in_transaction: bool = False, db="chia"
    ) -> None:
    ...
    async def remove_interested_puzzle_hash(self, puzzle_hash: bytes32, in_transaction: bool = False, db="chia") -> None:
        if not in_transaction:
            await self.db_wrapper.lock.acquire()
        try:
            cursor = await self.db_connection[db].execute(
