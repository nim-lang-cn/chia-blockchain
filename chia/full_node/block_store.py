...
class BlockStore:
    ...
    @classmethod
    async def create(cls, db_wrapper: DBWrapper, str="chia"):
        self = cls()
        ...
        # todo remove in v1.2
        await self.db[str].execute("DROP TABLE IF EXISTS sub_epoch_segments_v2")
        ...
        await self.db[str].execute("CREATE INDEX IF NOT EXISTS full_block_height on full_blocks(height)")
        await self.db[str].execute("CREATE INDEX IF NOT EXISTS is_block on full_blocks(is_block)")
        await self.db[str].execute("CREATE INDEX IF NOT EXISTS is_fully_compactified on full_blocks(is_fully_compactified)")

        await self.db[str].execute("CREATE INDEX IF NOT EXISTS height on block_records(height)")

        await self.db[str].execute("CREATE INDEX IF NOT EXISTS hh on block_records(header_hash)")
        await self.db[str].execute("CREATE INDEX IF NOT EXISTS peak on block_records(is_peak)")
        await self.db[str].execute("CREATE INDEX IF NOT EXISTS is_block on block_records(is_block)")
    ...
    async def add_full_block(self, header_hash: bytes32, block: FullBlock, block_record: BlockRecord, str="chia") -> None:
        self.block_cache.put(header_hash, block)
        cursor_1 = await self.db[str].execute(
        ...
        cursor_2 = await self.db[str].execute(
    ...
    async def persist_sub_epoch_challenge_segments(
        self, ses_block_hash: bytes32, segments: List[SubEpochChallengeSegment], str="chia"
    ) -> None:
    ...
    async def get_full_block(self, header_hash: bytes32, str="chia") -> Optional[FullBlock]:
        cached = self.block_cache.get(header_hash)
        if cached is not None:
            log.debug(f"cache hit for block {header_hash.hex()}")
            return cached
        log.debug(f"cache miss for block {header_hash.hex()}")
        cursor = await self.db[str].execute("SELECT block from full_blocks WHERE header_hash=?", (header_hash.hex(),))
        row = await cursor.fetchone()
        await cursor.close()
        if row is not None:
    ...
    async def get_full_block_bytes(self, header_hash: bytes32,str="chia") -> Optional[bytes]:
        cached = self.block_cache.get(header_hash)
        if cached is not None:
            log.debug(f"cache hit for block {header_hash.hex()}")
            return bytes(cached)
    ...
    async def get_block_records_by_hash(self, header_hashes: List[bytes32], str="chia"):
        """
        ...
        cursor = await self.db[str].execute(formatted_str, header_hashes_db)
        rows = await cursor.fetchall()
        await cursor.close()
    ...
    async def get_blocks_by_hash(self, header_hashes: List[bytes32], str="chia") -> List[FullBlock]:
        """
    ...
    async def get_block_record(self, header_hash: bytes32, str="chia") -> Optional[BlockRecord]:
        cursor = await self.db[str].execute(
    ...
    async def get_block_records_in_range(
        ...
        cursor = await self.db[str].execute(formatted_str)
        rows = await cursor.fetchall()
        await cursor.close()
    ...
    async def get_block_records_close_to_peak(
        ...
        res = await self.db[str].execute("SELECT * from block_records WHERE is_peak = 1")
    ...
    async def get_peak_height_dicts(self, str="chia") -> Tuple[Dict[uint32, bytes32], Dict[uint32, SubEpochSummary]]:
        """
        ...
        cursor = await self.db[str].execute("SELECT header_hash,prev_hash,height,sub_epoch_summary from block_records")
        rows = await cursor.fetchall()
        await cursor.close()
    ...
    async def set_peak(self, header_hash: bytes32, str="chia") -> None:
        ...
        cursor_1 = await self.db[str].execute("UPDATE block_records SET is_peak=0 WHERE is_peak=1")
        await cursor_1.close()
        cursor_2 = await self.db[str].execute(
    ...
    async def get_first_not_compactified(self, str="chia") -> Optional[int]:
